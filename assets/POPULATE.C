...
int insertBtrieve(BTRIEVEFILE *file, void *buffer, unsigned int length) {
  BTRIEVEDATA data;
  unsigned int status = 0xFFFF;
  char keyBuffer[8];
  ...
  data.data_buffer_length = length;
  data.key_number = 0;
  data.key_buffer = keyBuffer;
  data.key_buffer_length = 8;  
...
static int populate(BTRIEVEFILE *file) {
  char buffer[1024 + sizeof(POPULATE)];
  LPPOPULATE populate = (LPPOPULATE)buffer;
  unsigned char *variableData = (unsigned char*)(populate + 1);
  int i, j, r;

  // fill all variable data from 0->255
  for (i = 0; i < 1024; ++i) {
    variableData[i] = (unsigned char) i;
  }

  // populate all the rows
  for (i = 0; i < 1024; ++i) {
    populate->magic = 0xDEADBEEF;
    populate->key1 = i % 64;
    populate->key2 = i;

    r = insertBtrieve(file, populate, sizeof(POPULATE) + i);
    if (r) {
      return r;
    }
  }
